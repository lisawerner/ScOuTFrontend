...
var ReminderService = /** @class */ (function () {
    ...
    ReminderService.prototype.isActive = function (event, task) {
        //Get current Time
        var now = new Date();
        now.setHours(0, 0, 0);
        //console.log("Date now: ", now.toJSON());
        //Construct Eventbegin
        var beginDAY = event.begindate.substring(8, 10);
        var beginMONTH = event.begindate.substring(5, 7);
        var beginYEAR = event.begindate.substring(0, 4);
        ...
        var activ = now >= activationDate;
        //console.log("Fazit: Task ist aktiv=", activ);
        return activ;
    };
    ReminderService.prototype.isPast = function (event) {
        //Get current Time
        var now = new Date();
        now.setHours(0, 0, 0);
        //console.log("Date now: ", now.toJSON());
        ...
        var endMONTH = event.enddate.substring(5, 7);
        var endYEAR = event.enddate.substring(0, 4);
        var endDate = new Date();
        endDate.setHours(0, 0, 0);
        endDate.setFullYear(endYEAR, endMONTH - 1, endDAY);
        //console.log("Eventbegin as Date: ", beginDate.toJSON());
        var past = now > endDate;
        //console.log("Fazit: Task ist aktiv=", activ);
        return past;
    };
    ReminderService = tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
